...
int converteSensor(int value, int minValue, int maxValue)
{
	
	int interval = maxValue - minValue;
	value = value - minValue;

	//Valor mínimo 0 e valor máximo 500
	int percentage = (value * 100) / interval;
...
int leituraSensor()
{
	cout << "Informe um valor entre 0 a 830:" << endl;
	string writed;
...
int armazenaVetor(int* ponteiroVetor, int tamanhoVetor, int posAtual, int medida)
{
	if (posAtual < tamanhoVetor)
	{
		ponteiroVetor[posAtual] = medida;
		return posAtual+1;
	}
	else{
		return posAtual;
...
char* direcaoMenorCaminho(int* vetor, int* maiorDir)
{
	int maiorValor = 0;
	static char dir = 'D';

	if (vetor[0] >= maiorValor)
	{
		maiorValor = vetor[0];
		dir = 'D';
	}
	if (vetor[1] >= maiorValor)
	{
		maiorValor = vetor[1];
		dir = 'E';
	}
	if (vetor[2] >= maiorValor)
	{
		maiorValor = vetor[2];
		dir = 'F';
	}
	if (vetor[3] >= maiorValor)
	{
		maiorValor = vetor[3];
		dir = 'T';
	}
